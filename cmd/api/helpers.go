...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	...
	//limitting the size of request body to 1MB to prevent attacks - denial-of-service
	maxBytes := 1_048_576
	...
	if err != nil {
		...
		switch {
		//Use the erros.As() to check whether the error has the type
...
func (app *application) readString(qs url.Values, key string, defaultValue string) string {
	//Extracting the key from query string.
	s := qs.Get(key)

	//if no key exists then return default value:
	if s == "" {
		return defaultValue
	}
...
func (app *application) readCSV(qs url.Values, key string, defaultValue []string) []string {
	...
	//signature of Split() -> func Split(s, sep string) []string
}
...
func (app *application) readInt(qs url.Values, key string, defaultValue int, v *validator.Validator) int {
	//Extracting the string value from the query string:
	s := qs.Get(key)
	...
	//Try converting to int and if fails add error message to validator and return default value:
...
func (app *application) background(fn func()) {
	// Increment the WaitGroup counter to hault the graceful shutdown of server:
	app.wg.Add(1)
